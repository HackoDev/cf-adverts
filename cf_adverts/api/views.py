...
class SerializerSchemaMixin(object):
    """
    Mixin for mapping serializer classes.
    Would be used on different actions.
    """
    ...
    def get_serializer_class(self):
        """
        Return serializer class from `serializer_schema` dict.
...
class AdvertViewSet(SerializerSchemaMixin, ModelViewSet):
        ...
        if obj.is_available:
            ...
            if draft and draft.process_status != \
                ...
                raise self.permission_denied(
                    self.request,
                    _('Advert still is in moderation')
                )
    ...
    def send_to_moderation(self, request, pk):
        """
        Send advert to moderation.
        Advert status would be updated and blocked.
        """
        obj = self.get_object()
        if obj.process_status in [
            ...
            raise APIException(
                detail=_('Advert already moderated'),
